...
public class UpdateTrip extends JFrame {
	...
	public UpdateTrip() {
		...
		JLabel lblNewLabel = new JLabel("Tour Num");
		...
		JLabel lblNewLabel_2 = new JLabel("Guide ID");
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try {
					Utility ut=new Utility();
					String sql="update Tour_Trip set Guide_ID = ?, Destination = ? ,Price = ? ,Start_Time = ? ,Duration =? , Location = ?  where TourNo = ? and TripDate = ?";
					ut.conn.setAutoCommit(false);
					ut.pstmt=ut.conn.prepareStatement(sql);
					ut.pstmt.setString(1, V3.getText());
					ut.pstmt.setString(2, V4.getText());
					ut.pstmt.setString(3, V5.getText());
					ut.pstmt.setString(5, V7.getText());
					ut.pstmt.setTimestamp(4,java.sql.Timestamp.valueOf(V6.getText())); 
					ut.pstmt.setString(6, V8.getText());
					ut.pstmt.setString(7, V1.getText());
					ut.pstmt.setDate(8, java.sql.Date.valueOf(V2.getText())); 
					ut.pstmt.executeUpdate();
					...
					{
						ut.conn.rollback();
						JOptionPane.showMessageDialog(null,"Tour Trip Not Update");
					}
